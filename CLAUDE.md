...
- **목표:** LLM(OpenAI), Stability AI(Audio/Image), FFmpeg, YouTube API를 파이프라인으로 연결하여 매일 자정에 음악 쇼츠를 자동 업로드하는 시스템을 구축합니다.
- **사고방식 (CoT):** 코드를 작성하거나 수정하기 전, 반드시 터미널에 1) 현재 문제 상황, 2) 수정할 로직의 의도, 3) 예상되는 부작용을 먼저 브리핑하세요.

## 2. Directory Structure (이 구조를 엄격히 준수할 것)
- `/src/` : 모든 메인 비즈니스 로직
  - `llm_agent.py` (OpenAI API를 활용한 컨셉/프롬프트 기획)
  - `media_generator.py` (Stability AI를 활용한 오디오/이미지 미디어 생성)
  - `video_maker.py` (FFmpeg를 활용한 mp4 숏폼 영상 합성)
...
1. `llm_agent.py` -> 2. `media_generator.py` -> 3. `video_maker.py` -> 4. `uploader.py`
